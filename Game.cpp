...
void Game::Update(sf::Event& event, sf::RenderWindow& window)
{
    ...
    bool isSuperSenateOnfeild;
    for (int i = 0; i < maxEnemy; i++) {
        if (Senate[i].type == 1) {
            isSuperSenateOnfeild = true;
            break;
        }
        isSuperSenateOnfeild = false;
    }
    ...
    for (int i = 0; i < maxEnemy; i++) {
        if (Senate[i].enemyStatus == 0) {
            ...
            if (prob < 5) {
                gui.showText("[ Super Senate ] Appear !!", sf::Color::Red, 40, 400, 90, 3, sf::Color::White, 3);
